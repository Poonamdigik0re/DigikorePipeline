...
class Shot {
   constructor(data) {
       ...
       let bids = this.data.bids;
       let actuals = (this.data.actuals / 3600 / 8).toFixed(2);

       this.element = createElements(
           el('tr', {},
               el('td', {'class': 'text-overflow'}, this.data.show_name),
               el('td', {'class': 'text-overflow'}, this.data.sequence),
               el('td', {'class': 'text-overflow'}, this.data.name),
               el('td', {'class': 'text-overflow'}, this.data.department),
               el('td', {'class': 'text-overflow'}, this.data.status__name),
               el('td', {'class': 'text-overflow'}, this.data.resolution),
               el('td', {'class': 'align-center', 'style': 'width: 8em'}, bids || 0),
               el('td', {'class': 'align-center', 'style': 'width: 8em'}, actuals || 0),
               el('td', {'class': 'text-overflow'}, this.data.internal_approval),
               el('td', {'class': 'text-overflow'}, this.data.resolution),
               // el('td', {
               //     'class': 'align-center',
       ...
       //     window.open(`/shot_overview/${this.data.id}/`);
       // }
   }
}
...
const get_all_shots = (user_id = null) => {

    show_loading();
    post('get_all_shots/', {user_id: user_id}).then(resp => {
        console.log("resp:",resp)
       let shots_table = document.getElementById('project_shot_table');
       shots_table.innerText = '';

       let projects = [];
       let shot_statuses = [];

       let filter__projects = document.getElementById('filter__projects');
       let filter__shot__status = document.getElementById('filter__shot__status');
//
//        // clear filters
...
//
       resp.forEach(data => {
           let shot = new Shot(data);
           shots_table.append(shot.element);
           ALL_SHOTS.push(shot);
//
//            if (projects.indexOf(data.project__name) == -1) {
//                projects.push(data.project__name);
...
window.onload = async () => {-
    await init('Shots List');
    if (has_permission('shots_list')) {
        get_all_users();
        // get_projects();
    }
